...
const ROOM_PHOTO_DEFAULT_BATCH_PAGES = 10;
...
export class RecNetService extends EventEmitter {
  ...
  async moveLibraryTo(
    ...
    try {
      ...
      const emitOrchestration = (
        partial: Partial<LibraryMoveProgress> &
          Pick<LibraryMoveProgress, 'phase'>
      ) => {
        onProgress({
          phase: partial.phase,
      ...
      if (error instanceof LibraryMoveCancelledError) {
        ...
        try {
          ...
          logMove(
            'Cancel cleanup: destination partial copy removed where possible.'
          );
        } catch {
      ...
      if (!copyAndVerifySucceeded) {
        ...
        try {
          ...
          logMove(
            'Failure cleanup: emptied destination folder where possible.'
          );
        } catch {
    ...
    for (const account of this.normalizeAccounts(accounts)) {
      ...
      if (folderMeta?.accountId === accountId) {
        localProfileImagePath =
          ...
          (await this.pathExistsWithContent(
            folderMeta.ownerLocalProfileImagePath
          ))
            ? folderMeta.ownerLocalProfileImagePath
            : await this.resolveMetadataEntryPath(
  ...
  private async syncUserLibraryMetadataAssets(
    ...
    await this.writeFolderMeta(accountDir, accountId, {
      ownerLocalProfileImagePath:
        ownerAssets?.profile?.absolutePath ??
        existing?.ownerLocalProfileImagePath,
      ownerLocalBannerImagePath:
        ownerAssets?.banner?.absolutePath ??
        existing?.ownerLocalBannerImagePath,
    });
    return true;
  }
  ...
  private async syncCreatorSharedMetadataAssets(
    ...
    throwIfMetadataSyncAborted(signal);
    const creator =
      await this.loadCreatorPlayerResultForEvents(creatorAccountId);
    if (!creator) {
      return false;
    }
  ...
  private async syncEventFolderMetadataAssets(
    ...
    if (syncAccounts && (await fs.pathExists(accountsJsonPath))) {
      try {
        const rawAccounts = (await fs.readJson(
          accountsJsonPath
        )) as PlayerResult[];
        await this.syncCentralAccountMetadataAssets(
  ...
  async syncMetadataLibraryAssets(params?: {
    ...
    try {
      ...
      console.log(`Metadata library sync failed: ${(error as Error).message}`);
    }
  ...
  private async writeFolderMeta(
    ...
    next.ownerLocalProfileImagePath =
      patch.ownerLocalProfileImagePath ?? existing?.ownerLocalProfileImagePath;
  ...
  private getEventDirectory(creatorAccountId: string, eventId: string): string {
    return path.join(
      this.getCreatorEventsDirectory(creatorAccountId),
  ...
  private async readRoomFolderMeta(
    ...
    try {
      ...
      const parsed = (await fs.readJson(metaPath)) as Partial<RoomFolderMetaV1>;
  ...
  async fetchAndSaveBulkData(
    ...
    try {
      ...
      const accountDir =
        storageContext?.directory ??
        path.join(this.getResolvedOutputRoot(), accountId);
  ...
  async loadEventAlbumPhotos(params: {
    ...
    const photos = this.normalizePhotos(
      (await fs.readJson(photosJsonPath)) as Photo[]
    );
    ...
    return photos
      ...
      .map(photo => ({
        ...
        localFilePath: path.join(
          photosDir,
          `${this.sanitizeFileStem(photo.Id)}.jpg`
        ),
      }));
  }
  ...
  async loadUserAccountsData(accountId: string): Promise<PlayerResult[]> {
    ...
    const jsonPath = path.join(
      accountDir,
      `${normalizedAccountId}_accounts.json`
    );
    if (!(await fs.pathExists(jsonPath))) {
      return [];
    }
  ...
  async downloadEventPhotos(
    ...
    const eventIds = Array.from(
      new Set(
        (params.eventIds || []).map(id => this.normalizeId(id)).filter(Boolean)
      )
    );
      ...
      for (let eventIndex = 0; eventIndex < eventIds.length; eventIndex++) {
        ...
        if (!event) {
          ...
          downloadResults.push({
            ...
            status: 'failed',
            reason:
              'Event metadata was not found. Refresh events and try again.',
          });
          continue;
        }
  ...
  async lookupRoomById(roomId: string, token?: string): Promise<RoomDto> {
    await this.ensureSettingsLoaded();
    const cleanedRoomId = this.normalizeId(roomId);
    if (!cleanedRoomId) {
    ...
    try {
      const room = this.normalizeRooms(
        await this.roomsController.fetchBulkRooms([cleanedRoomId], token)
      )[0];
      if (!room?.RoomId) {
        throw new Error('Room not found');
      }
      ...
      throw new Error(
        `Failed to lookup room by ID: ${(error as Error).message}`
      );
    }
  }
  ...
  async listAvailableRooms(): Promise<AvailableRoom[]> {
    ...
    try {
      ...
      console.log(
        `Failed to list available rooms: ${(error as Error).message}`
      );
    }
