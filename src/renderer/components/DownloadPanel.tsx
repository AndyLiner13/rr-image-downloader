...
export const DownloadPanel: React.FC<DownloadPanelProps> = ({
  ...
  const [eventKnownCreatorAccountId, setEventKnownCreatorAccountId] = useState<
    string | undefined
  >(undefined);
  ...
  useEffect(() => {
    ...
    const { creatorAccountId, eventIds, usernameHint } = eventDownloadPrefill;
  ...
  const handleDownload = async () => {
    ...
    await onDownload(
      ...
      roomPhotoSort,
      selectedEventIds,
      libraryMode === 'event' ? eventKnownCreatorAccountId : undefined
    );
  };

  const handleDownloadMyRooms = async () => {
    const outputPath = settings.outputRoot || '';
    if (!outputPath.trim() || !onDownloadMyRooms) {
      return;
    }

    const cleanedToken = cleanToken(token);
    onOpenChange(false);
    await onDownloadMyRooms(
      cleanedToken,
      outputPath,
      {
        forceAccountsRefresh,
  ...
  const eventCardColumns = Math.max(
    1,
    Math.floor(
      (eventListWidth + EVENT_CARD_GAP) /
        (EVENT_CARD_MIN_WIDTH + EVENT_CARD_GAP)
    )
  );
  ...
  const eventStartRow = Math.max(
    0,
    Math.floor(eventListScrollTop / (EVENT_CARD_ROW_HEIGHT + EVENT_CARD_GAP)) -
      1
  );
            ...
            <div className="space-y-4">
              ...
              {onDownloadMyRooms && (
                <Button
                  ...
                  variant="secondary"
                  className="w-full"
                  disabled={!outputSavePathAllowed || isDownloading}
                  onClick={() => void handleDownloadMyRooms()}
                >
                  <Download className="mr-2 h-4 w-4" />
                  Download my room&apos;s photos
                </Button>
              )}
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              {eventsLoading ? (
                ...
                >
                  <div
                    style={{
                      paddingTop: eventListPaddingTop,
                      paddingBottom: eventListPaddingBottom,
                    }}
                  >
                    <div
                    ...
                    >
                      {visibleEventCards.map(event => {
                        const selected = selectedEventIds.includes(
                          event.eventId
                        );
                        return (
                          <div
