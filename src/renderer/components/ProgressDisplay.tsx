...
export const ProgressDisplay: React.FC<ProgressDisplayProps> = ({
  ...
  const roomPhotoQueue = progress.roomPhotoQueue;
  const hasTotals = progress.total > 0 && !isConfirming && !roomPhotoQueue;
  const isCancelled =
  ...
  const currentRoomPercent = Math.min(
    Math.max(Math.round(roomPhotoQueue?.currentBatchProgress ?? 0), 0),
    100
  );
  const hasCurrentRoomProgress =
    !!roomPhotoQueue &&
    roomPhotoQueue.currentBatchTotal !== undefined &&
    roomPhotoQueue.currentBatchTotal > 0;
  const downloadPhaseMessage = roomPhotoQueue
    ? 'Downloading room photo queue...'
  ...
  return (
    <Card className={cardToneClass}>
      <CardHeader>
        <CardTitle className="flex items-center justify-between gap-2">
          ...
          <span className="flex items-center gap-1">
            <span className={`hidden text-xs font-medium sm:inline ${statusToneClass}`}>
              {statusText}
            </span>
            <Button
              variant="ghost"
      ...
      {!isCollapsed && (
        ...
        {roomPhotoQueue && (
          <div className="space-y-3 rounded border border-border/70 bg-muted/20 px-3 py-3">
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm font-medium">Room photo queue</p>
                <p className="text-xs text-muted-foreground">
                  {roomPhotoQueue.currentRoomName
                    ? `Room ${roomPhotoQueue.currentRoomIndex ?? roomPhotoQueue.roomsCompleted + 1} of ${roomPhotoQueue.totalRooms}: ^${roomPhotoQueue.currentRoomName}`
                    : `${roomPhotoQueue.totalRooms} room${roomPhotoQueue.totalRooms === 1 ? '' : 's'} queued`}
                </p>
              </div>
              <span className="text-xs text-muted-foreground">
                {roomPhotoQueue.roomsCompleted}/{roomPhotoQueue.totalRooms}{' '}
                rooms complete
              </span>
            </div>
            <div className="rounded border border-border/70 bg-background/70 px-3 py-2 text-xs text-muted-foreground">
              Overall image total is discovered as Rec.net pages are scanned.
              The bar below is only for the current page/download batch.
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-5">
              <div className="rounded border border-border/70 bg-background/70 px-2 py-1">
                Discovered: {formatCount(roomPhotoQueue.photosDiscovered)}
              </div>
              <div className="rounded border border-border/70 bg-background/70 px-2 py-1">
                Downloaded: {formatCount(roomPhotoQueue.newDownloads)}
              </div>
              <div className="rounded border border-border/70 bg-background/70 px-2 py-1">
                On disk: {formatCount(roomPhotoQueue.alreadyDownloaded)}
              </div>
              <div className="rounded border border-border/70 bg-background/70 px-2 py-1">
                Failed: {formatCount(roomPhotoQueue.failedDownloads)}
              </div>
              <div className="rounded border border-border/70 bg-background/70 px-2 py-1">
                Batches: {formatCount(roomPhotoQueue.batchesCompleted)}
              </div>
            </div>
            {roomPhotoQueue.currentRoomName && (
              <div className="space-y-2 rounded border border-border/70 bg-background/70 px-3 py-2">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                  <span className="text-xs font-medium">
                    Current room: ^{roomPhotoQueue.currentRoomName}
                  </span>
                  <span className="text-xs text-muted-foreground">
                    {formatCount(
                      roomPhotoQueue.currentRoomPhotosDiscovered ?? 0
                    )}{' '}
                    discovered in this room
                  </span>
                </div>
                {hasCurrentRoomProgress ? (
                  <>
                    <div className="flex justify-between text-xs text-muted-foreground">
                      <span>
                        {roomPhotoQueue.currentBatchLabel ??
                          `Batch ${roomPhotoQueue.currentBatch ?? 1}`}
                      </span>
                      <span>
                        {formatCount(roomPhotoQueue.currentBatchCurrent ?? 0)} /{' '}
                        {formatCount(roomPhotoQueue.currentBatchTotal ?? 0)} (
                        {currentRoomPercent}%)
                      </span>
                    </div>
                    <Progress
                      value={currentRoomPercent}
                      className={`h-2 ${progressToneClass}`}
                    />
                  </>
                ) : (
                  <p className="text-xs text-muted-foreground">
                    Preparing current room...
                  </p>
                )}
              </div>
            )}
            {(roomPhotoQueue.message || roomPhotoQueue.currentBatch) && (
              <p className="text-xs text-muted-foreground">
                {roomPhotoQueue.message ??
                  `Scanning batch ${roomPhotoQueue.currentBatch}`}
              </p>
            )}
          </div>
        )}
