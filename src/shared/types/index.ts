...
export interface Progress {
  ...
  roomPhotoQueue?: RoomPhotoQueueProgress;
}

export interface RoomPhotoQueueProgress {
  totalRooms: number;
  ...
  currentBatchCurrent?: number;
  currentBatchTotal?: number;
  currentBatchProgress?: number;
  currentBatchLabel?: string;
  currentRoomPhotosDiscovered?: number;
  photosDiscovered: number;
  newDownloads: number;
  alreadyDownloaded: number;
  failedDownloads: number;
  hasMoreForCurrentRoom?: boolean;
  message?: string;
}
...
export interface MyRoomsManifestResult {
  sourcePath: string;
  rooms: RoomDto[];
}
